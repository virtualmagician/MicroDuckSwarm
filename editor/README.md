...
- **Lanes** — one group per role (collapse with ▾, rename with ✎, remove with ✕, add with **+ role**), with sub-lanes `locomotion` (vx/vy/vyaw), `head` (neck_pitch/head_pitch/head_yaw/head_roll), `pose` (z/roll/pitch + `active` band), `mouth` (open), and `events`. Each curve lane spans exactly the validation limits of its fields, so anything touching the lane edge is at the limit. Curves are drawn by sampling the same sampler the duck uses (step / linear / smooth), so what you see is what plays. The small glyph under a keyframe shows its interp (`/` linear, `~` smooth, `⌐` step).
  - **Solo / Mute (rehearsal tools)** — **S** and **M** buttons on each role's lane header. **Solo** plays only that role; every other role holds its neutral standing pose at its mark. **Mute** holds one or more specific roles neutral while the rest of the cast plays normally. Mixing-desk rules: at most one role is soloed at a time, soloing a different role replaces it, and solo overrides mute outright — the muted set is untouched underneath, so un-soloing simply resumes reading whatever mutes were set (mutes flipped *while* soloed take effect the moment you un-solo). A held-neutral duck is visibly dimmed on stage — dark body, no trail, no event/name label — and its lane header dims to match; the soloed role's own header gets an amber accent bar instead. `⌥S` (Option/Alt+S) toggles solo on the role currently selected in the lane header (click its name first). **This is rehearsal state, never show content** — like the splitter position and the name-label toggle, it lives in this browser's `localStorage` and is never written into the `.duckshow` file (pure logic + neutral-pose derivation tested in `tests/rehearsal.test.mjs`).
...
| `S` | toggle snap |
| `⌥S` (`Alt`+`S`) | solo/un-solo the role currently selected in the lane header |
| `+` `−` `0` | zoom in / out / fit |
