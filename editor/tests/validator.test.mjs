...
const ROLLER = [{ name: 'p', mode: 'roller', file: 'x.onnx', sha256: 'abc' }];
...
test('mode events: undeclared is a warning only; declared is clean', () => {
  const undeclared = validate(show({ events: [{ t: 1.0, mode: 'not_declared' }] }, { policies: ROLLER }));
  assert.deepEqual(undeclared, [{
    severity: 'warning', role: 'lead', track: 'events', t: 1.0,
    message: "mode event references 'not_declared', not declared in requires.policies",
  }]);
  assert.deepEqual(validate(show({ events: [{ t: 1.0, mode: 'roller' }] }, { policies: ROLLER })), []);
});
...
test('mode event overlapping nonzero locomotion within +/-0.5 s warns', () => {
  ...
  const zeroNearby = show({ locomotion: [{ t: 0.0, vx: 0.0 }, { t: 2.0, vx: 0.0 }], events: [{ t: 1.0, mode: 'roller' }] }, { policies: ROLLER });
  assert.deepEqual(warnings(zeroNearby), []);

  const far = show({ locomotion: [{ t: 0.0, vx: 0.1 }, { t: 1.0, vx: 0.0 }], events: [{ t: 5.0, mode: 'roller' }] }, { policies: ROLLER });
  assert.deepEqual(warnings(far), []);

  // Window edge: locomotion becomes nonzero exactly at t+0.5 -> still inside the closed window.
  const edge = show({ locomotion: [{ t: 0.0, vx: 0.0, interp: 'step' }, { t: 5.5, vx: 0.1 }], events: [{ t: 5.0, mode: 'roller' }] }, { policies: ROLLER });
  assert.equal(warnings(edge).length, 1);
  // Locomotion held after its last keyframe counts too; but at/after duration it is zeroed.
  const held = show({ locomotion: [{ t: 0.0, vx: 0.1 }], events: [{ t: 9.0, mode: 'roller' }] }, { policies: ROLLER });
  assert.equal(warnings(held).length, 1);
  const afterEnd = show({ locomotion: [{ t: 0.0, vx: 0.1 }], events: [{ t: 10.5, mode: 'roller' }] }, { policies: ROLLER });
...
test('issue order matches Python: duration, then per role (sort, interp, limits, events, density, modes, overlap, servo)', () => {
  ...
  const perRole = [
    ...
    "mode event references 'x', not declared in requires.policies",
  ];
