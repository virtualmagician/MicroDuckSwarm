...
**Saving writes the file back.** Three routes, in order of preference, and the editor picks the best one available:

1. **`POST /api/save`**, when the show was opened from the repo and `scripts/editor_server.py` is serving. Writes straight back to the file it came from: no dialog, no download, works in every browser, and survives a page reload because the path travels with the show rather than living in a JS handle.
2. **The File System Access API** (`showSaveFilePicker`), for a show opened by file picker or drag-and-drop. The handle is remembered for the session, so only the first save prompts. Chrome and Edge only; Safari does not implement it.
3. **A download**, when neither is available. The status line says plainly that you have to move the file yourself.

The save endpoint is narrower than the bake one on purpose, because it *writes*: the target must be an existing `.duckshow.json` **under `shows/`**, excluding `shows/fixtures/` (those are validator test data, several deliberately invalid, and an editor overwriting them would corrupt the parity suite). The body must parse as a `duckshow/N` document. The server writes atomically through a temporary file in the same directory and renames, so an interrupted write cannot truncate a show you were performing from.
...
**Intent curves, and the drift diff.** The `paths` toggle draws each role's whole dead-reckoned path: the complete shape the `locomotion` track describes, future included. That is a different object from a trail, which shows only the last few seconds because it encodes recency; the retired 2D stage drew the full curve for every role and the 3D viewer never did. The path is a flat ribbon on the floor, 2 cm wide, because WebGL draws lines one pixel wide whatever the display, and a one-pixel hairline in grey on a light floor was invisible. With no bake loaded the ribbon takes the role's own colour at reduced opacity: it is the only path there is. With a bake loaded it turns neutral grey and sits under the simulated trail, because a diff is only legible if the reference recedes. Marks stay draggable underneath it, and **⌥-drag (or ⇧-drag) turns a duck**, pointing its heading at the cursor. The path is drawn, not dragged: it is integrated from the locomotion keyframes, so it changes when a keyframe in the timeline or a mark on the stage does.
