...
**Unsaved edits are refused, not baked around.** A bake reads the show from disk; an edited-but-unsaved document would produce a cache whose `show.sha256` could never be honestly checked against what's on screen. **Create Preview** refuses outright in that case ("save your edits first") — the same policy **Play Baked…** already enforces when hash-checking a picked cache file against `state.showText`, not a second policy invented for this button. Saving first and pressing the button again is the way through.

**Everything else still works.** The terminal route — `cd tools/bake && .venv/bin/python3 bake_show.py <show> <out>` — is unchanged and always available; the button is a convenience over it, not a replacement. Serve the repo any other way — plain `python3 -m http.server`, a different static server, or open the HTML from `file://` — and the editor is exactly the plain static page it always was: the kinematic preview and **Play Baked…** work precisely as before, `/api/capabilities` simply isn't there to answer, and **Create Preview** explains that plainly on the button itself instead of failing silently or not appearing at all.
